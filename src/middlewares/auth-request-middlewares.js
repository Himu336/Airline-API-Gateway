...
async function checkAuth(req, res, next) {
    try {
        const isAuthenticated = UserService.isAuthenticated(req.headers['x-access-token']);
        if(response){
            req.user = response;
            next();
        }
...
async function isAdmin(req, res, next) {
    ...
    if(!response) {
        return res
            ...
            .json(new AppError('You are not authorized to perform this action', StatusCodes.UNAUTHORIZED));
    }
    next();
};
